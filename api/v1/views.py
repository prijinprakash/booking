...
class BookingInfoListView(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response({"error": 'Invalid date format.It should be of the form `YYYY-MM-DD`'},
                            status=status.HTTP_400_BAD_REQUEST)

        if start_date and end_date and end_date < start_date:
            return Response({"error": 'Checkout date can not be less than check in date'},
                            status=status.HTTP_400_BAD_REQUEST)
    ...
    def can_make_reservation(start_date, end_date, booking_info_obj):
        """
        function that validates whether a reservation can be made for a listing in the given date range.
        @param start_date: check in date object (if query param 'check_in') or None
        @param end_date: check out date object (if query param 'check_out') or None
        @param booking_info_obj: BookingInfo object

        returns: True if a reservation can be made in the date range else False
        """
        ...
        if booking_info_obj.listing.listing_type == Listing.HOTEL:
            # at least one hotel room must be available in the date range
            ...
            if len(reservations) < no_of_hotel_rooms_in_this_booking:
                # not all hotels are reserved in the date range
                return True
            return False
